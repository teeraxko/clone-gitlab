...
{
    public class ConnectLoan2SAP
    {
        public string GetLoanSAPDataFile(GLJournal journal, DateTime postingDate)
        {
            ...
            //List<GLDebit> detailsData = journal.Debits;
            List<GLitem> detailsData = journal.Debits;

            // Base function
            templateList = sapConnectBase.GetSAPTemplateFromDB(BTS2SAPTransferTypeEnums.GLTransferType.GL_LOAN.ToString());
            ...
            {
                BTS2SAPModel basAMT = templateList.Where(x => x.MiscItemCode.Trim().ToUpper().Equals("BASAMT")).FirstOrDefault();
            ...
            //string filename = "Loan_" + DateTime.Now.ToString("yyMMddHHmmss") + ".csv";
