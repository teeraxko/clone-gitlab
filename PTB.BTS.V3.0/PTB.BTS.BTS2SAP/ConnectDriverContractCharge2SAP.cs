...
{
    public class ConnectDriverContractCharge2SAP
    {
        public string GetDriverContractChargeSAPDataFile(BTS2BizPacList resultList, string postingDate)
        {
            ...
            for (int i = 0; i < resultList.Count; i++)
            {
                ...
                DriverContractChargeBP headData = (DriverContractChargeBP)resultList[i];

                BTS2BizPacDetailAR detailAR = (BTS2BizPacDetailAR)headData.GetDetail(0);

                //Base function
                dataList = sapConnectBase.GetSAPTemplateFromDB("AR_D_CHARGE");
                ...
                foreach (BTS2SAPModel line in dataList)
                {
                    ...
                    detailAR.ItemDescription = "Driving service fee in " + BTS2BizPacCommon.GetStringMonthYear(DateTime.ParseExact(postingDate, "ddMMyyyy", CultureInfo.InvariantCulture));
            ...
            //string filename = "PTB_BTS_AR_" + DateTime.Now.ToString("yyMMddHHmmss") + ".csv";
