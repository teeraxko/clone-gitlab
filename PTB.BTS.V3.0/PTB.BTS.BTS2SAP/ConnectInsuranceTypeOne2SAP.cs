...
{
    public class ConnectInsuranceTypeOne2SAP
    {
        public string GetInsuranceTypeOneSAPDataFile(BTS2BizPacList resultList, DateTime postingDate)
        {
            ...
            for (int i = 0; i < resultList.Count; i++)
            {
                ...
                InsuranceTypeOneBP headData = (InsuranceTypeOneBP)resultList[i];

                BTS2BizPacDetailAP detailAP = (BTS2BizPacDetailAP)headData.Details[0];

                // Base function
                dataList = sapConnectBase.GetSAPTemplateFromDB(BTS2SAPTransferTypeEnums.APTransferType.AP_VINSUR.ToString());
                ...
                foreach (BTS2SAPModel line in dataList)
                {
                    switch (line.MiscItemCode.ToUpper())
                    {
                        ...
                        case "VINSUR":
                            line.BizPacRefNo = headData.BizPacRefNo;
                            line.AmountInDocumentCurrency = headData.Month1Amount.ToString("F");
                            line.ItemText = (line.ItemText.Trim().Equals("1") ? headData.BizPacRemark1 : string.Empty);
                            break;
                        case "PREP01":
                            line.BizPacRefNo = headData.BizPacRefNo;
                            line.AmountInDocumentCurrency = (headData.BaseAmount - headData.Month1Amount).ToString("F");
                            line.ItemText = (line.ItemText.Trim().Equals("1") ? headData.BizPacRemark1 : "");
                            break;
                        case "VATAMT":
                            line.BizPacRefNo = headData.BizPacRefNo;
